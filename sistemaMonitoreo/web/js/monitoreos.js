...
function initMap() {
  var waypointsArray = [
    ...
    directionsService.route(request, function(response, status) {
      if (status === 'OK') {
         var numcordenates= response.routes[0].overview_path.length;
          
          var positions = response.routes[0].overview_path; // Get the current position along the route
          for (let i = 0; i < positions.length; i++) {
            console.log("latitude="+positions[i].lat()+", longitude="+positions[i].lng())
          }
